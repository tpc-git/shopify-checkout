// QStash delayed customer-SMS callback.
// Fires SMS_DELAY_SECONDS after the SMS job is scheduled; sends via Quo if the
// checkout is still unfinished and eligible. Returns 200 on intentional skips
// (no retry), 500 on transient failures (QStash retries).
...
export async function POST(req: Request): Promise<Response> {
  ...
  const outcome = await sendScheduledCustomerSms(token);

  if (outcome.status === 'error') {
    console.error(`[qstash sms ${token}]`, outcome.error);
